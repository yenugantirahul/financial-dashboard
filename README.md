...
All routes are prefixed with `/api`. Requests requiring authentication must include a valid session token via the `Authorization: Bearer <token>` header or a session cookie.
...
|----------------|--------------------------------------------------|--------------------------------------|
| `DATABASE_URL` | PostgreSQL connection string                     | `postgresql://user:pass@localhost:5432/findb` |
