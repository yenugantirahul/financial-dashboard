...
const toNumber = (value: unknown) => Number(String(value ?? 0));
const formatMonthKey = (date: Date) => date.toISOString().slice(0, 7);

type MonthlyBucket = { month: string; income: number; expense: number; net: number };

router.get("/", authenticate, authorize("ADMIN", "ANALYST", "VIEWER"), async (req: AuthenticatedRequest, res) => {
  try {
